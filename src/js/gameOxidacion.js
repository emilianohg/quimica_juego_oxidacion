...
var Opcion = React.createClass({
	...
	render: function render() {
		return React.createElement(
			'p',
			{ key: this.props.key },
			React.createElement('input', { name: this.props.name, id: this.props.id, value: this.props.value, type: 'radio' }),
			React.createElement(
				'label',
...
var Elemento = React.createClass({
	...
	chooseOption: function chooseOption(correct) {
	...
	chooseElement: function chooseElement() {
		var elem,
				...
				React.createElement(
					...
					this.state.opciones.map(function (opcion, key) {
						return React.createElement(
							'div',
							null,
							React.createElement(Opcion, { value: opcion, id: key, name: parent.state.nombre, key: key })
						);
					})
				)
			)
...
var BtnCheck = React.createClass({
	...
	check: function check() {
		...
		if (typeof r_intento != 'string') {
			...
			if (r_correcta == r_intento) alert('Correcto');else alert('Error, la respuesta correcta es ' + r_correcta);

			//Cambiar de elemento
			$('#box-elemento').html('');
			React.render(React.createElement(Elemento, null), $('#box-elemento')[0]);
		}
	},
	render: function render() {
		return React.createElement(
...
$(document).ready(function () {
	$.getJSON('src/data/elementos.json', function (data) {
		...
		React.render(React.createElement(Elemento, null), $('#box-elemento')[0]);
		React.render(React.createElement(BtnCheck, null), $('#box-controles')[0]);
	});
});